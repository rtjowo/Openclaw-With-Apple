...
3. **标题日期格式规则（重要）**：因为 iPhone 快捷指令无法使用提醒事项的时间字段，所以当用户提到了日期时，**必须将日期写入标题文本**中作为前缀：
   - 用户说**含日期的话**（今天、明天、后天、下周X、具体日期等）→ 标题格式为 `"X月X日 做某事"`
     - "明天去洗车"（假设明天是3月22日）→ 标题 `"3月22日 洗车"`
     - "今天2点开会"（假设今天是3月21日）→ 标题 `"3月21日 开会"`
     - "下周三去体检"（假设下周三是3月26日）→ 标题 `"3月26日 体检"`
   - 用户说**不含日期的话** → 标题**不加日期前缀**，直接写事项
     - "去洗车" → 标题 `"洗车"`
     - "记得买牛奶" → 标题 `"买牛奶"`
   - ⚠️ 注意：日期前缀用 `X月X日` 格式（不带年份、不带星期），与事项之间用一个空格分隔
...
8. **待办同步**：本地数据在 `~/.openclaw/tasks.json`，每晚 21:00 通过 launchd 自动上传
9. **Tasks/Notes iPhone 端**：Tasks Import 和 Notes Import 22:15 执行，各自读取对应文件夹中的 JSON 后删除文件
