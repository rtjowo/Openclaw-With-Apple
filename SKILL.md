...
> 当用户问某个功能怎么用、或者第一次使用某功能时，**必须先引导用户完成 iPhone 端配置**，不能直接教用法就完事。

用户问怎么用时，按以下模板回复（根据用户问的功能选择对应部分）：

```
要使用这个功能，需要先在 iPhone 上完成配置：

📱 第 1 步：导入快捷指令

用 iPhone Safari 打开以下链接，点击「添加快捷指令」：

  📋 提醒事项同步：https://www.icloud.com/shortcuts/9054c0236adb4909b3dbf72fa58b4933
  📝 备忘录同步：https://www.icloud.com/shortcuts/56d84868591f4233b7d362c83fb71d59
  🏥 健康数据采集：https://www.icloud.com/shortcuts/4a5dff0072a6463a9a1fef47d6ec13e2
...
导入后，每个快捷指令都手动运行一次：
  - 首次运行会创建 iCloud Drive 中的文件夹（Tasks/Notes/Health）
  - 首次运行会请求权限授权，全部点「允许」
  - Health 快捷指令需要额外手动授权健康数据（见下方说明）

⏰ 第 3 步：设置 iPhone 自动化

快捷指令 App → 底部「自动化」→ 右上角 + →「特定时间」：

  📋 Tasks Import  → 每天 22:05 → 关闭「运行前询问」
  📝 Notes Import  → 每天 22:06 → 关闭「运行前询问」
  🏥 Health Daily Export → 每天 23:00 → 关闭「运行前询问」

这样每天自动运行，无需手动操作。

🔐 Health 额外步骤（仅健康功能需要）：

  1. iPhone →「设置」→「快捷指令」→「高级」→ 开启「允许共享大量数据」
  2. 如果运行时提示没权限：
     打开快捷指令编辑模式 → 展开每个「查找健康样本」→ 逐个授权
     或：「设置」→「隐私与安全性」→「健康」→「快捷指令」→ 全部开启
...
如果用户只问某一个功能（如"备忘录怎么用"），只展示该功能相关的快捷指令链接和自动化步骤，不用全部展示。
