...
2️⃣ Apple ID 主密码（可选）— 用于照片、iCloud Drive、查找设备、Health 健康分析
   → 就是你登录 Apple ID 的密码

如果你只需要日历功能，提供应用专用密码即可。
如果还需要照片/文件/设备/健康分析，请同时提供主密码。
```
...
```bash
python icloud_auth.py login       # 使用用户提供的主密码登录
python icloud_auth.py status      # 验证 session
```

> 如果触发双重认证，提示用户查看 iPhone 上的验证码并输入。
...
2. iCloud session 过期时，提示用户重新 `python icloud_auth.py login`
...
| iCloud 照片/Drive/设备/Health | Apple ID 主密码 | (不需要环境变量) | 用户直接提供，通过 `icloud_auth.py login` 登录 |
...
2. **iCloud Session**：缓存在 `~/.pyicloud/`，过期需重新 `login`
