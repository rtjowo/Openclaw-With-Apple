...
📱 第 2 步：在 iPhone 上创建快捷指令

运行以下命令查看详细创建步骤：
python scripts/generate_tasks_shortcut.py

简化版（2 分钟搞定）：
  1. 新建快捷指令，添加「获取文件」→ /Shortcuts/Tasks/tasks_sync.json
  2. 「获取文件中的文本」→「从输入中获取词典」→「获取词典值」键: tasks
  3. 「针对每一项重复」→「获取词典值」键: title →「添加新提醒事项」
  4. 「删除文件」（删源文件防重复导入）→「显示通知」

⏰ 第 3 步：设置 iPhone 自动化

快捷指令 App →「自动化」→ + →「特定时间」→ 每天 22:05
→ 运行「Tasks Import」→ 关闭「运行前询问」

⚠️ 设置 22:05（比上传时间 22:00 晚 5 分钟），确保文件已上传再读取。
...
1. **实时记录**：用户在对话中提到任何待办事项、提醒、备忘时，立即调用 `tasks_tool.py add` 写入
2. **智能解析**：从自然语言中提取日期（"明天"、"下周三"）、时间（"下午2点"）、优先级（"重要"→high）
3. **目标判断**：带时间提醒的 → `--target reminder`；纯笔记/长文 → `--target note`
4. **主动确认**：添加后简短确认（"已加：明天 14:00 开会 ⏰"），不要冗长
...
用户对话 → tasks_tool.py add → ~/.openclaw/tasks.json (本地)
                                  ...
                                  22:00 launchd 定时
                                        │
                                        ▼
                              iCloud Drive/Shortcuts/Tasks/
                              ├── tasks_2026-03-11.json  ← 归档(永久保留)
                              └── tasks_latest.json      ← 快捷指令读取
                                        │
                                  22:05 iPhone 自动化
                                        │
                                  ┌─────┴─────┐
...
8. **Tasks iPhone 端**：快捷指令设置为 22:05 执行，读取 `tasks_latest.json` 后删除它（日期归档文件 `tasks_YYYY-MM-DD.json` 永久保留）
