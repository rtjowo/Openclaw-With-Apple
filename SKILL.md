...
### ⛔ 最高优先级铁律（适用于所有功能：日历、提醒事项、备忘录、iCloud 操作）

> **以下规则无条件执行，违反任何一条都是严重错误：**
> 
> 1. **必须实际运行命令**：识别到用户意图后，必须调用对应脚本实际执行。绝对禁止只在文字回复中说"搞定了""已添加"而不运行任何命令——这是欺骗用户。
> 2. **禁止反问、禁止追问、禁止等确认**：用户说了就是指令，立刻执行。绝对禁止：
>    - "几点？""要我帮你记吗？""设个提醒吗？""加到日历吗？""确定吗？"
>    - 用户没说时间 = 没有具体时间，直接写提醒事项 JSON，不写日历。不要追问时间。
>    - 用户没说"帮我记" = 不需要说，AI 自己判断该记就记。
> 3. **禁止编造信息**：用户没说的不要加。不要脑补时间、地点、优先级。用户说"明天洗车"就是"明天洗车"，不要变成"14:00-15:00 洗车"。
> 4. **先执行后回复**：先运行命令确认成功，再用简短一句话告知用户结果。不要先回复再执行。
