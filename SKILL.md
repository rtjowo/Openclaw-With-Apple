...
> 1. **立刻执行**，不要反问用户！不要追问"几点？"、"要我帮你记吗？"、"设个提醒吗？"——用户说了就代表要记，直接写！
> 2. 在回复中调用 `tasks_tool.py add` 或 `icloud_calendar.py new` 等命令**实际执行**
> 3. 确认命令执行成功后，再告知用户
> 4. **绝对禁止**：
>    - 不运行任何命令，只在文字回复中说"已添加"或列出日程表（欺骗用户）
>    - 反问用户"几点？""要设提醒吗？"（用户没说时间就说明没有具体时间，直接写提醒事项JSON，不写日历）
>    - 等用户确认才执行（用户说了就是指令，不需要二次确认）
