...
2. **双写规则（核心）**：用户提到行动事项时，同时做两件事：
   - **所有事项 → 待办 JSON**：不管有没有具体时间，全部通过 `tasks_tool.py add` 写入待办（提醒事项/备忘录）
   - **有具体时间的 → 额外写日历**：如果用户说了几点（如"明天2点开会"、"后天早上8点打太极拳"），**同时**调用 `icloud_calendar.py new` 写入日历
   - **没有具体时间的 → 只写待办**：如"明天晚上去开会"（只有大概时段没有准确时间），只写待办 JSON，不写日历
   - 示例："明天2点开会、明天晚上去开会、后天早上8点打太极拳" → 
     - 待办 JSON：3 条全写入
     - 日历：只写"明天2点开会"和"后天早上8点打太极拳"（有具体时间）
3. **目标判断**：
   - 有明确时间或需要执行的动作 → `--target reminder`（提醒事项）
   - 信息记录、笔记、想法、纪要 → `--target note`（备忘录）
4. **智能解析**：从自然语言中提取日期（"明天"、"下周三"）、时间（"下午2点"）、优先级（"重要"→high）
5. **主动确认**：添加后简短确认，格式如：
   - "已记：明天 14:00 开会 ⏰📅"（⏰=待办 📅=已加日历）
   - "已记：明天晚上开会 ⏰"（只有待办，无日历）
